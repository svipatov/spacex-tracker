...
export default {
  [ACTIONS.FETCH_PAST] ({ commit }) {
    return getPastLaunches().then(response => addLaunches(commit, response.data))
  },
  [ACTIONS.FETCH_UPCOMING] ({ commit }) {
    return getUpcomingLaunches().then(response => addLaunches(commit, response.data))
  },
  [ACTIONS.FETCH_LAUNCH] ({ commit }, flight) {
    return getLaunch({ flight }).then(response => addLaunches(commit, response.data))
  }
}
